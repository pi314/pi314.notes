...
Shell 指令有時會需要用到特別的 escape sequence (例如 ``sed`` 的 group 需要 ``\(`` ``\)``)，
使用 ``r'''`` 避免讓 Python 發出 ``DeprecationWarning`` 或 ``SyntaxWarning``

--------

若需要互相嵌入程式碼的話 (例如 shell → Python → shell) (在這裡先停一下，為什麼會需要？)，
